...
struct hlslpp_nodiscard double1
{
	...
	template <int X>
	hlslpp_inline double1(const dswizzle1<X> &s) : vec(s.template swizzle<X, 0>()) {}

	// double1(const int1& i);

	hlslpp_inline double1 &operator=(const double1 &f)
	{
		vec = f.vec;
		return *this;
	}
...
struct hlslpp_nodiscard double2
{
	...
	template <int X, int Y>
	hlslpp_inline double2(const dswizzle2<X, Y> &s) : vec(s.template swizzle<X, Y, 0, 1>()) {}

	// double2(const int2& i);

	hlslpp_inline double2 &operator=(const double2 &f)
	{
		vec = f.vec;
		return *this;
	}
