...
struct hlslpp_nodiscard float8
{
	...
	hlslpp_inline float8 &operator=(float8 &&f) hlslpp_noexcept
	{
		vec = f.vec;
		return *this;
	}
...
hlslpp_inline float8 &operator+=(float8 &f1, const float8 &f2)
{
	f1 = f1 + f2;
	return f1;
}
...
hlslpp_inline float8 &operator++(float8 &f)
{
	f = f + float8(f8_1);
	return f;
}
...
hlslpp_inline float8 operator++(float8 &f, int)
{
	float8 tmp = f;
	f = f + float8(f8_1);
	return tmp;
...
template <typename T>
hlslpp_inline hlslpp_enable_if_return(T, float8) operator+(const float8 &f1, T f2) { return f1 + float8(f2); }

template <typename T>
hlslpp_inline hlslpp_enable_if_return(T, float8) operator+(T f1, const float8 &f2) { return float8(f1) + f2; }
...
hlslpp_inline void sincos(const float8 &f, float8 &s, float8 &c)
{
	s = float8(_hlslpp256_sin_ps(f.vec));
	c = float8(_hlslpp256_cos_ps(f.vec));
}
