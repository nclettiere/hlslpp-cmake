...
namespace internal
{
	...
	hlslpp_inline float round_float(float f)
	{
		...
		return abs_frac <= 0.5f ? trunc : f >= 0.0f ? trunc + 1.0f
													: trunc - 1.0f;
	}
};
...
{
	return vector_float4(v1.x > max.x ? max.x : v1.x < min.x ? min.x
															 : v1.x,
						 v1.y > max.y ? max.y : v1.y < min.y ? min.y
															 : v1.y,
						 v1.z > max.z ? max.z : v1.z < min.z ? min.z
															 : v1.z,
						 v1.w > max.w ? max.w : v1.w < min.w ? min.w
															 : v1.w);
}

hlslpp_inline vector_float4 _hlslpp_sat_ps(const vector_float4 &v)
{
	return vector_float4(v.x > 1.0f ? 1.0f : v.x < 0.0f ? 0.0f
														: v.x,
						 v.y > 1.0f ? 1.0f : v.y < 0.0f ? 0.0f
														: v.y,
						 v.z > 1.0f ? 1.0f : v.z < 0.0f ? 0.0f
														: v.z,
						 v.w > 1.0f ? 1.0f : v.w < 0.0f ? 0.0f
														: v.w);
}
...
{
	uint32_t ix = v1.ix & v2.ix;
	uint32_t iy = v1.iy & v2.iy;
	uint32_t iz = v1.iz & v2.iz;
	uint32_t iw = v1.iw & v2.iw;
...
hlslpp_inline void _hlslpp_store2_ps(float *p, const vector_float4 &v)
{
	p[0] = v.x;
	p[1] = v.y;
}
hlslpp_inline void _hlslpp_store3_ps(float *p, const vector_float4 &v)
{
	p[0] = v.x;
	p[1] = v.y;
	p[2] = v.z;
}
hlslpp_inline void _hlslpp_store4_ps(float *p, const vector_float4 &v)
{
	p[0] = v.x;
	p[1] = v.y;
	p[2] = v.z;
...
hlslpp_inline void _hlslpp_load2_ps(float *p, n128 &v)
{
	v.x = p[0];
	v.y = p[1];
}
hlslpp_inline void _hlslpp_load3_ps(float *p, n128 &v)
{
	v.x = p[0];
	v.y = p[1];
	v.z = p[2];
}
hlslpp_inline void _hlslpp_load4_ps(float *p, n128 &v)
{
	v.x = p[0];
	v.y = p[1];
	v.z = p[2];
...
{
	return vector_int4(v1.x > max.x ? max.x : v1.x < min.x ? min.x
														   : v1.x,
					   v1.y > max.y ? max.y : v1.y < min.y ? min.y
														   : v1.y,
					   v1.z > max.z ? max.z : v1.z < min.z ? min.z
...
{
	return vector_int4(v1.x > 1 ? 1 : v1.x < 0 ? 0
											   : v1.x,
					   v1.y > 1 ? 1 : v1.y < 0 ? 0
											   : v1.y,
					   v1.z > 1 ? 1 : v1.z < 0 ? 0
...
hlslpp_inline void _hlslpp_store2_epi32(int32_t *p, const vector_int4 &v)
{
	p[0] = v.x;
	p[1] = v.y;
}
hlslpp_inline void _hlslpp_store3_epi32(int32_t *p, const vector_int4 &v)
{
	p[0] = v.x;
	p[1] = v.y;
	p[2] = v.z;
}
hlslpp_inline void _hlslpp_store4_epi32(int32_t *p, const vector_int4 &v)
{
	p[0] = v.x;
	p[1] = v.y;
	p[2] = v.z;
...
hlslpp_inline void _hlslpp_load2_epi32(int32_t *p, vector_int4 &v)
{
	v.x = p[0];
	v.y = p[1];
}
hlslpp_inline void _hlslpp_load3_epi32(int32_t *p, vector_int4 &v)
{
	v.x = p[0];
	v.y = p[1];
	v.z = p[2];
}
hlslpp_inline void _hlslpp_load4_epi32(int32_t *p, vector_int4 &v)
{
	v.x = p[0];
	v.y = p[1];
	v.z = p[2];
...
{
	return vector_uint4(v1.x > max.x ? max.x : v1.x < min.x ? min.x
															: v1.x,
						v1.y > max.y ? max.y : v1.y < min.y ? min.y
															: v1.y,
						v1.z > max.z ? max.z : v1.z < min.z ? min.z
...
{
	return vector_uint4(v1.x > 1 ? 1 : v1.x < 0 ? 0
												: v1.x,
						v1.y > 1 ? 1 : v1.y < 0 ? 0
												: v1.y,
						v1.z > 1 ? 1 : v1.z < 0 ? 0
...
hlslpp_inline void _hlslpp_store2_epu32(uint32_t *p, const vector_uint4 &v)
{
	p[0] = v.x;
	p[1] = v.y;
}
hlslpp_inline void _hlslpp_store3_epu32(uint32_t *p, const vector_uint4 &v)
{
	p[0] = v.x;
	p[1] = v.y;
	p[2] = v.z;
}
hlslpp_inline void _hlslpp_store4_epu32(uint32_t *p, const vector_uint4 &v)
{
	p[0] = v.x;
	p[1] = v.y;
	p[2] = v.z;
...
hlslpp_inline void _hlslpp_load2_epu32(uint32_t *p, vector_uint4 &v)
{
	v.x = p[0];
	v.y = p[1];
}
hlslpp_inline void _hlslpp_load3_epu32(uint32_t *p, vector_uint4 &v)
{
	v.x = p[0];
	v.y = p[1];
	v.z = p[2];
}
hlslpp_inline void _hlslpp_load4_epu32(uint32_t *p, vector_uint4 &v)
{
	v.x = p[0];
	v.y = p[1];
	v.z = p[2];
